...
本书是《CATs 微服务架构设计书 v1.0》§21「与详细设计的衔接」中承诺的接口详细设计文档，覆盖 15 个微服务（8 个核心业务服务 + translation-core + 6 个媒体处理服务）的完整对外/对内契约：REST/gRPC API 清单、Kafka 事件 schema、错误码、鉴权方式、版本管理策略。数据库表结构见《CATs 数据库设计书 v2.0》，服务内部模块划分见《CATs 模块设计书 v2.0》。
