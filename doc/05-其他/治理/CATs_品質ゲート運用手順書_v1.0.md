# CATs 品質ゲート運用手順書 v1.1（基线升级）

> **文档编号**：CATs-IMPL-055  
> **フェーズ**：55/56 静的解析（SAST）/ コードレビュー（CR）  
> **关联任务**：150 任务 #55、#56、#128（QA Review）  
> **版本**：v1.1（基线升级 WT-H2：msrv 锁定 1.98.0）  
> **创建日**：2026-08-20
> **修订日**：2026-08-26  
> **作者**：QA + 架构师 + Rust Lead + DBA（worker 代签 per DEC-008）
...
| **v1.1** | **2026-08-26** | **QA + 架构师 + Rust Lead + DBA（worker 代签 per DEC-008）** | **基线升级(WT-H2)**:`msrv = "1.79"` → **`msrv = "1.98.0"`**(2026-08-26 锁定,见 `CATs_技术基线_v1.0 §1`);注释更新 "CI 实际用 1.98.0"** |
...
msrv = "1.98.0"  # 最低支持版本，CI 实际用 1.98.0（2026-08-26 锁定，见 CATs_技术基线_v1.0 §1）
```
