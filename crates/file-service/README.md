...
原始文件/中间产物/最终产物的存取、预签名 URL、版本管理
...
```bash
docker build -f deploy/docker/Dockerfile.rust --build-arg CRATE_NAME=file-service -t file-service:0.1.0 .
```

## Helm 部署

```bash
helm lint deploy/helm/file-service
helm template deploy/helm/file-service
```
