...
mod tests {
    ...
    #[test]
    fn route_to_resource_action_v1_files() {
        assert_eq!(
            route_to_resource_action("/v1/files", "POST"),
            Some((Resource::File, Action::Create))
        );
        assert_eq!(
            route_to_resource_action("/v1/files", "GET"),
            Some((Resource::File, Action::Read))
        );
        assert_eq!(
            route_to_resource_action("/v1/files/abc", "GET"),
            Some((Resource::File, Action::Read))
        );
        assert_eq!(
            route_to_resource_action("/v1/files/abc", "DELETE"),
            Some((Resource::File, Action::Delete))
        );
    }
    ...
    async fn enforce_anonymous_is_rejected() {
        ...
        let req = TestRequest::default().to_http_request();
        let result = enforce(&checker, &req, "/v1/files", "GET").await;
    ...
    #[tokio::test]
    async fn enforce_user_can_read_files() {
        let checker = Arc::new(RbacChecker::new());
        let req = TestRequest::default()
            .insert_header((header::AUTHORIZATION, "Bearer cats-role:User"))
            .to_http_request();
        let result = enforce(&checker, &req, "/v1/files", "GET").await;
        assert!(result.is_ok(), "User should be able to read files");
    }

    #[tokio::test]
    async fn enforce_user_cannot_delete_files() {
        let checker = Arc::new(RbacChecker::new());
        let req = TestRequest::default()
            .insert_header((header::AUTHORIZATION, "Bearer cats-role:User"))
            .to_http_request();
        let result = enforce(&checker, &req, "/v1/files/abc", "DELETE").await;
        let (status, _body) = result.expect_err("User cannot delete files");
