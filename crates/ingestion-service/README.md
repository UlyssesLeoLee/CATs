...
| 项目 | 内容 |
|---|---|
| Crate 名 | `ingestion-service` |
| 阶段 | 阶段二 |
| 默认端口 | 8091（由 env `BIND_ADDR` 覆盖） |
| 数据边界 | `task_db` |
| 镜像 | `harbor.cats.internal/cats/ingestion-service:0.1.0` |

## 概述

统一接收各媒体类型原始文件，识别媒体类型并拆解为处理任务
...
```powershell
cargo test -p ingestion-service
```

## 容器化

```bash
docker build -f deploy/docker/Dockerfile.rust --build-arg CRATE_NAME=ingestion-service -t ingestion-service:0.1.0 .
```

## Helm 部署

```bash
helm lint deploy/helm/ingestion-service
helm template deploy/helm/ingestion-service
```
