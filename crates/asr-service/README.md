...
| Crate 名 | `asr-service` |
| 阶段 | 阶段二 |
| 默认端口 | 8092（由 env `BIND_ADDR` 覆盖） |
| 数据边界 | `无独立库` |
| 镜像 | `harbor.cats.internal/cats/asr-service:0.1.0` |

## 概述

语音转写（faster-whisper），输出带时间戳的转写文本
...
```powershell
cargo test -p asr-service
```

## 容器化

```bash
docker build -f deploy/docker/Dockerfile.rust --build-arg CRATE_NAME=asr-service -t asr-service:0.1.0 .
```

## Helm 部署

```bash
helm lint deploy/helm/asr-service
helm template deploy/helm/asr-service
```
